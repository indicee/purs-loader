...
module.exports = function purescriptLoader(source, map) {
  ...
  var glob = [].concat(loaderOptions.src);
  var _iteratorNormalCompletion = true;
  var _didIteratorError = false;
  var _iteratorError = undefined;

  try {
    for (var _iterator = globby.sync(glob)[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
      var file = _step.value;

      debugVerbose('Adding', file, 'as a dependency');
      this.addDependency(file);
      var jsFile = path.join(path.dirname(file), path.basename(file, path.extname(file))) + '.js';
